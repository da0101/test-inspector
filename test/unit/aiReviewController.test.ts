...
test('AI reviewer includes related source context for test-file reviews and skips unreadable related files', async () => {
  let prompt = '';
  const provider = providerFixture({
    complete: async (input) => {
      ...
      return {
        ...
        text: JSON.stringify({
          verdictAlignsWithEvidence: true,
          explanation: 'The related source context was included.',
          evidenceAnchors: [{ lineNumber: 1, excerpt: 'import assert', issue: 'target file was reviewed' }],
          suggestedFix: { summary: 'Keep related context grounded.' },
        }),
        modelUsed: 'gpt-test',
      };
    },
  ...
  const target = {
    ...caseFixture(),
    target: { kind: 'test' as const, path: path.join(process.cwd(), 'test/unit/aiReviewController.test.ts'), projectId: 'node:/repo' },
    evidence: {
      signals: [],
      relatedTests: [
        { path: path.join(process.cwd(), 'test/unit/aiReviewController.test.ts'), weaknesses: [] },
        { path: path.join(process.cwd(), 'missing-related.ts'), weaknesses: [] },
      ],
    },
  };
  const source = {
    ...caseFixture(),
    target: { kind: 'source' as const, path: path.join(process.cwd(), 'src/services/aiReviewController.ts'), projectId: 'node:/repo' },
    evidence: { signals: [], relatedTests: [{ path: target.target.path, weaknesses: [] }] },
  };

  const review = createAiReviewer(new Map([['openai', provider]]) as never, outputMock());
  const result = await review(target, { ...bundleFixture(), cases: [target, source] });

  assert.equal(result.status, 'accepted');
  assert.match(prompt, /RELATED/);
  assert.match(prompt, /src\/services\/aiReviewController\.ts/);
});
